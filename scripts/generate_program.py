...
def main(args):
    ...
    train_data_path = args.data_path + args.dataset_name + "/test_data.json"
    with open(train_data_path, 'r') as f:
        train_data = json.load(f)
    train_data = train_data if args.num_eval_samples < 0 else train_data[:args.num_eval_samples]

    # load model
    logger.info("loading model......")
    program_generate = Program_Generate(args)
    program_generate.batch_generate_program(logger, train_data)
