...
def remove_complements(text):
    # 去除（）、[]以及{}包含的部分
    pattern = r'(_-LRB-.*?-RRB-)|(_-LCB-.*?-RCB-)|(_-LSB-.*?-RSB-)'
    return re.sub(pattern, '', text)

def untokenize(text):
    text = remove_complements(text)
...
def main(args):
    ...
    out_path = args.out_path + args.dataset + "_" + args.mode + "_count_unmatched_keyword_without_complement.json"
