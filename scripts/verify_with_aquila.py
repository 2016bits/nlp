...
class Program_Generate:
    ...
    def batch_generate_program(self, logger, datasets):
        ...
        for inst in tqdm(datasets):
            ...
            if len(input_text) > self.args.max_aquila_tokens:
                evidence_len = self.args.max_aquila_tokens - len(claim) - 25
                input_text = f"{evidence[:evidence_len]}\nBased on the above information, is it true that {claim}? true, false or unknown? The answer is: "

            pred = self.ai_model.generate_answer(input_text, temperature)
            flag = False
            out = pred.split()
            if out[0]:
                if out[0] in label_map:
                    pred = label_map[out[0]]
                    flag = True
            else:
                if out[1] in label_map:
                    pred = label_map[out[1]]
                    flag = True

            for _label in label_map:
                if flag:
                    break
                if _label in pred.lower():
                    pred = label_map[_label]
                    flag = True
                    break
            if not flag:
            ...
            # except:
            #     pred = random.sample([0, 1, 2], 1)[0]
            #     logger.info('Error in predicting for example: {}'.format(inst['id']))
        
            # # map output to label_map
            # pred = pred.lower().strip()
            # label_map = {'true': 1, 'false': 0, 'yes': 1, 'no': 0, 'unknown': 2, 'un': 2, "it is not possible to tell": 2,
...
def main(args):
    log_path = args.log_path + args.dataset_name + '_verify_with_aquila.log'
    ...
    program_generate = Program_Generate(args, logger)
    outputs = program_generate.batch_generate_program(logger, dataset)

    # save outputs
    save_path = args.save_path + args.dataset_name + "_test_verify_with_aquila_results.json"
...
if __name__ == "__main__":
    ...
    # model
    parser.add_argument('--max_aquila_tokens', type=int, default=2048)
