...
from utils.generate_batch_data import BatchData
    ...
    for _, data in enumerate(data_loader):
        ...
        for pred in pred_labels:
            if pred not in label_id_list:
                ...
                logger.info(f"The predicted label is not in label_id_list, its prediction is {text}")
...
def main(args):
    ...
    train_batch = BatchData(tokenizer, args.db_path, args.max_t5_tokens, args.batch_size)
    batched_train_data = train_batch.batch_list_instances(train_dataset[:2000])
    ...
    dev_batch = BatchData(tokenizer, args.db_path, args.max_t5_tokens, 1)
    batched_dev_data = dev_batch.batch_list_instances(dev_dataset[:200])
    
    # load model
    logger.info("loading model......")
    model = T5ForConditionalGeneration.from_pretrained(args.model_name, cache_dir=args.cache_dir).to('cuda')
    ...
    if args.use_Adafactor and args.use_AdafactorSchedule:
        ...
        lr_scheduler = get_linear_schedule_with_warmup(optimizer=optimizer,
                                                       ...
                                                       num_training_steps=len(batched_train_data) * args.epoch)
    else:
    ...
    for epoch in range(args.epoch):
        ...
        data_loader = tqdm(batched_train_data, file=sys.stdout)
        ...
        dev_macro_f1 = validate(model, args.device, batched_dev_data, label_id_list, epoch, logger, tokenizer)
...
if __name__ == '__main__':
    ...
    #     args.device = 'cuda:' + ','.join(map(str, args.gpu))
    # else:
    #     args.device = "cpu"
    if torch.cuda.is_available() and args.gpu is not None:
        device = 'cuda:' + ','.join(map(str, args.gpu))
        args.device = "cuda"
