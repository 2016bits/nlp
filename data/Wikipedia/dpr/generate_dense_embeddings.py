...
from scripts.data.biencoder_data import BiEncoderPassage
from scripts.models import init_biencoder_components
from scripts.options import set_cfg_params_from_state, setup_cfg_gpu, setup_logger
...
def gen_ctx_vectors(
    ...
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        if not only_title:
            ...
            batch_token_tensors = [
                tensorizer.text_to_tensor(ctx[1].title) for ctx in batch
            ]
...
def main(cfg: DictConfig):
    ...
    print("saved model keys ={}".format(saved_state.model_dict.keys()))
