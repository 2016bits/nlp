...
from scripts.data.biencoder_data import (
    BiEncoderPassage,
    normalize_passage,
    get_dpr_files,
...
from scripts.utils.data_utils import normalize_question
...
class RetrieverData(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        # dpr_wiki -> self.file='data.wikipedia_split.psgs_w100' ->self.data_file=['/home/jhxiao/share_test/DPR-main/downloads/data/wikipedia_split/psgs_w100.tsv']
        # policy -> self.file='data.policy_split.policy_part' 
        assert (
...
class WikiData(torch.utils.data.Dataset):
    ...
    def load_data(self):
        self.data_files = [f for f in iter_files(self.file)]
        self.file = self.data_files

class JsonlCtxSrc(WikiData):
    """read data from files and save in dict"""
    def __init__(
        self,
        file: str,
        title: str = "",
    ...
    def load_data_to(self, ctxs: Dict[object, BiEncoderPassage]):
        """save data in dict: key is sample_id, value is BiEncoderPassage"""
        super().load_data()
        count = 0
        for filename in tqdm(self.file):
            with open(filename) as f:
                for line in f:
                    ...
                    title = doc['id']
                    if self.id_prefix:
                        sample_id = self.id_prefix + str(count)
                    else:
                        sample_id = str(count)
                    ctxs[sample_id] = BiEncoderPassage(passage, title)
                    count += 1
        print('Read {} docs.'.format(count))

class ClaimSrc(RetrieverData):
    def __init__(
        self,
        file: str,
        selector: DictConfig = None,
...
def get_document(evidence_list):
    """get documents and evidence sentences from given evidence"""
    doc = []
    sentence = []
    for evidence in evidence_list:
        if evidence not in sentence:
            sentence.append(evidence)
...
class FCSample:
    def __init__(self, claim: str, id: int, document: List, sentence: List, label: str):
        self.claim = claim
        self.id = id
        ...
        with open(self.file) as ifile:
            ...
            for inst in dataset:
                ...
                if label == 'NOT ENOUGH INFO':
                    ...
                    doc, sentence = get_document(evidence)
                data.append(FCSample(claim, id, doc, sentence, label))
