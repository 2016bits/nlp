...
from scripts.data.retriever_data import TableChunk
...
def calculate_matches(
    ...
    scores = processes.map(get_score_partial, questions_answers_docs) # 这是什么神奇的写法.
...
def check_answer(questions_answers_docs, tokenizer, match_type) -> List[bool]:
    ...
    answers, (doc_ids, doc_scores) = questions_answers_docs #(answers,([top_k_docs],[top_k_scores])
    # match type for string
    global dpr_all_documents
    hits = []

    for i, doc_id in enumerate(doc_ids):
        doc = dpr_all_documents[doc_id]
        text = doc[0] #doc:class BiEncoderPassage; doc[0] : text ,doc[1] :  title
        ...
        if match_type == "kilt":
            ...
            answer_found = True # match type is "string" or "regx"
